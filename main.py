...
def getAccuracy(args):
    ...
    pd.DataFrame(svc_cnf).to_csv(os.path.join("confusion_matrix",BaseLanguage+f"svc_pca{pca_level}.csv"))
    ...
    pd.DataFrame(rf_cnf).to_csv(os.path.join("confusion_matrix",BaseLanguage+f"rf_cnf{pca_level}.csv"))
    ...
    pd.DataFrame(votingClf_cnf).to_csv(os.path.join("confusion_matrix",BaseLanguage+f"voting_pca{pca_level}.csv"))
...
if __name__ == "__main__":
    ...
    for BaseLanguage in BaseLanguages:
        try:
            ...
            svc_cnf = confusion_matrix(y_test,svc_pred)
            pd.DataFrame(svc_cnf).transpose().to_csv(os.path.join("confusion_matrix",BaseLanguage+"svc_base.csv"))
            svc_clf = classification_report(y_test, svc_pred, output_dict=True)
            ...
            rf_cnf = confusion_matrix(y_test,rf_pred)
            pd.DataFrame(rf_cnf).transpose().to_csv(os.path.join("confusion_matrix",BaseLanguage+"rf_base.csv"))
            rf_clf = classification_report(y_test, rf_pred,output_dict=True)
            ...
            pd.DataFrame(voting_cnf).transpose().to_csv(os.path.join("confusion_matrix",BaseLanguage+"voting_base.csv"))
